  ...
  const handleImportFromFile = async () => {
    try {
      const selected = await openDialog({
        filters: [
          { name: 'opclaude Agent', extensions: ['opclaude.json', 'json'] },
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      const path = await save({
        defaultPath: `${agent.name.toLowerCase().replace(/\s+/g, '-')}.opclaude.json`,
        filters: [
          { name: 'opclaude Agent', extensions: ['opclaude.json'] }
        ]
      });
