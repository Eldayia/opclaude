# Opclaude - NixOS Build & Development Commands
...
web: build-frontend
    cd src-tauri && cargo run --bin opclaude-web

# Run web server on custom port
web-port PORT: build-frontend
    cd src-tauri && cargo run --bin opclaude-web -- --port {{PORT}}
...
info:
    @echo "🚀 Opclaude - Claude Code GUI Application"
